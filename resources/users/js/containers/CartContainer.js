...
const CartContainer = ({cartList, total, symbol, removeFromCart}) => (
     <li  className="onhover-div mobile-cart"><div className="cart-qty-cls">{cartList.length}</div>
        <Link to={`/users/cart`}><img src={`/users/assets/images/icon/cart.png`} className="img-fluid" alt=""/>
        ...
        <ul className="show-div shopping-cart">
            ...
            <li>
                <div className="buttons">
                    <Link to={`/users/cart`} className="view-cart">view cart</Link>
                    <Link to={`/users/checkout`} className="checkout">checkout</Link>
                </div>
            </li></div>
                    :
